package tutorial5;

public class calcularVelocidadeMedia {
    public static void main(String[] args) {

        System.out.println("===============================");

        System.out.println("Cálculo da Velocidade Média");

        double distanciaPercorrida = 150;
        double tempoGasto = 1.5;
        double velocidadeMedia = distanciaPercorrida / tempoGasto;
        System.out.println("A distânca percorrida foi de " + distanciaPercorrida + " Km");
        System.out.println("O tempo gasto foi de " + tempoGasto + " horas");
        System.out.println("Para encontrar a velocidade média, dividimos a distância percorrida pelo tempo gasto");
        System.out.println("A velocidade média é " + velocidadeMedia + " Km/h");

        System.out.println("===============================");
    }
}